...
class FinanceController < ApplicationController
  # before_filter :login_required,:Schooldatum_settings_for_finance
  ...
  def donation_receipt_pdf
    @donation = FinanceDonation.find(params[:id])
    @currency_type = Schooldatum.find_by_config_key("CurrencyType").config_value
  ...
  def pdf_fee_structure
    @student = Student.find(params[:id])
    @institution_name = Schooldatum.find_by_config_key("InstitutionName")
    @institution_address = Schooldatum.find_by_config_key("InstitutionAddress")
    @institution_phone_no = Schooldatum.find_by_config_key("InstitutionPhoneNo")
    @currency_type = Schooldatum.find_by_config_key("CurrencyType").config_value
    @fee_collection = FinanceFeeCollection.find params[:id2]
    @fee_category = FinanceFeeCategory.find(@fee_collection.fee_category_id,:conditions => ["is_deleted IS NOT NULL"])
    @fee_particulars = @fee_collection.fees_particulars(@student)
  ...
  def batch_wise_discount_create
    unless params[:fee_collection].blank?
      ...
      @fee_discount = BatchFeeDiscount.new(params[:fee_discount])
      @fee_discount.errors.add :base, "Fee Category cant be blank"
      @error = true
    end
  ...
  def category_wise_fee_discount_create
    unless params[:fee_collection].blank?
      ...
      @fee_discount = StudentCategoryFeeDiscount.new(params[:fee_discount])
      @fee_discount.errors.add :base, "Batch cant be blank"
      @error = true
    end
  end
  ...
  def student_wise_fee_discount_create
    ...
    unless (params[:fee_discount][:finance_fee_category_id]).blank?
      ...
      unless (params[:students]).blank?
        ...
        admission_no.each do |a|
          ...
          unless s.nil?
            if FeeDiscount.find_by_type_and_receiver_id('StudentFeeDiscount',s.id,:conditions=>"finance_fee_category_id = #{@fee_category.id}").present?
              @error = true
              @fee_discount.errors.add :base, "Discount already exists for admission number - #{a}"
            end
            unless (s.batch_id == @fee_category.batch_id)
              @error = true
              @fee_discount.errors.add :base, "#{a} does not belong to Batch #{@fee_category.batch.full_name}"
            end
          else
            @error = true
            @fee_discount.errors.add :base, "#{a} is invalid admission number"
          end
        end
        unless @error
          admission_no.each do |a|
        ...
        @error = true
        @fee_discount.errors.add :base, "Admission can't be blank"
      end
    else
      @error = true
      @fee_discount.errors.add :base, "Fees Category can't be blank"
    end
  end
